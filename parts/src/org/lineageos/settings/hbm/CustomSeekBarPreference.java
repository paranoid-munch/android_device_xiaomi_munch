...
public class CustomSeekBarPreference extends Preference implements SeekBar.OnSeekBarChangeListener {
    ...
    public void onDependencyChanged(Preference dependency, boolean disableDependent) {
        ...
        if (mResetImageView != null) mResetImageView.setEnabled(!disableDependent);
        if (mPlusImageView != null) mPlusImageView.setEnabled(!disableDependent);
        if (mMinusImageView != null) mMinusImageView.setEnabled(!disableDependent);
    }
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        mResetImageView.setOnClickListener(view -> Toast.makeText(getContext(), getContext().getString(R.string.custom_seekbar_default_value_to_set, getTextValue(mDefaultValue)),
                Toast.LENGTH_LONG).show());
        mResetImageView.setOnLongClickListener(view -> {
            setValue(mDefaultValue, true);
            return true;
        });
        mMinusImageView.setOnClickListener(view -> setValue(mValue - mInterval, true));
        mMinusImageView.setOnLongClickListener(view -> {
            setValue(mMaxValue - mMinValue > mInterval * 2 && mMaxValue + mMinValue < mValue * 2 ? Math.floorDiv(mMaxValue + mMinValue, 2) : mMinValue, true);
            return true;
        });
        mPlusImageView.setOnClickListener(view -> setValue(mValue + mInterval, true));
        mPlusImageView.setOnLongClickListener(view -> {
