...
function blob_fixup() {
    case "${1}" in
        ...
        vendor/lib64/libMIAIHDRhvx_interface.so | vendor/lib64/libarcsoft_hdrplus_hvx_stub.so)
            [ "$2" = "" ] && return 0
            for sym in remote_handle_close remote_handle_invoke remote_handle_open; do
                "${PATCHELF}" --clear-symbol-version "${sym}" "${2}" 2>/dev/null || true
            done
            ;;
        vendor/lib64/libarcsoft_super_night_raw.so)
            [ "$2" = "" ] && return 0
            for sym in rpcmem_alloc rpcmem_free rpcmem_to_fd; do
                "${PATCHELF}" --clear-symbol-version "${sym}" "${2}" 2>/dev/null || true
            done
            ;;
        vendor/lib64/vendor.qti.hardware.camera.postproc@1.0-service-impl.so)
            [ "$2" = "" ] && return 0
            sed -i 's|\x9A\x0A\x00\x94|\x1F\x20\x03\xD5|g' "${2}"
            ;;
