...
function blob_fixup() {
    case "${1}" in
        ...
        vendor/etc/libnfc-nci.conf)
            [ "$2" = "" ] && return 0
            grep -q content "LEGACY_MIFARE_READER=1" "${2}" || echo "LEGACY_MIFARE_READER=1" >> "${2}"
            ;;
        ...
        vendor/lib64/libMIAIHDRhvx_interface.so|vendor/lib64/libarcsoft_hdrplus_hvx_stub.so)
            [ "$2" = "" ] && return 0
            "${PATCHELF_0_8}" --clear-symbol-version "remote_handle_close" "${2}"
            "${PATCHELF_0_8}" --clear-symbol-version "remote_handle_invoke" "${2}"
            "${PATCHELF_0_8}" --clear-symbol-version "remote_handle_open" "${2}"
            ;;
        vendor/lib64/libarcsoft_super_night_raw.so)
            [ "$2" = "" ] && return 0
            "${PATCHELF_0_8}" --clear-symbol-version "rpcmem_alloc" "${2}"
            "${PATCHELF_0_8}" --clear-symbol-version "rpcmem_free" "${2}"
            "${PATCHELF_0_8}" --clear-symbol-version "rpcmem_to_fd" "${2}"
            ;;
        vendor/lib64/vendor.qti.hardware.camera.postproc@1.0-service-impl.so)
            [ "$2" = "" ] && return 0
            echo "9a0a0094" | xxd -r -p | sed -i 's/\x9a\x0a\x00\x94/\x1f\x20\x03\xd5/g' "${2}"
            ;;
